...
import AdvancedSystemSettings from "../../components/AdvancedSystemSettings";
...
const SuperAdminDashboard = () => {
  ...
  const fetchAnalyticsData = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        "http://localhost:5004/api/analytics/dashboard",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  useEffect(() => {
    ...
    socket.on("user_registered", (newUserData) => {
    ...
    return {
      lineChartData: {
        ...
        datasets: [
          {
            label: "Users",
            data: analyticsData.revenueData?.map((item) => item.userCount) || [
              0, 0, 0, 0, 0, 0, 0,
            ],
            borderColor: "rgb(59, 130, 246)",
            backgroundColor: "rgba(59, 130, 246, 0.1)",
          ...
          {
            label: "Revenue (ETB)",
            data: analyticsData.revenueData?.map(
 ...
 useEffect(() => {
  ...
  // Wrap fetchData in useCallback to stabilize dependencies
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!usersResp.ok) {
        console.error("Failed to fetch users:", usersResp.statusText);
        setUsers([]); // Set an empty array to avoid undefined state
        return;
      }
      ...
      if (!todosResp.ok) {
        console.error("Failed to fetch todos:", todosResp.statusText);
        return;
      }
      ...
      const recentUsers = usersData
        .filter((u) => u.id !== user?.id) // Exclude current super admin user
      ...
      const analyticsActivities =
        analyticsResp?.recentActivities?.map((activity) => ({
  ...
  const createUser = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/auth/super-admin/create-user",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const confirmUserCreation = async (payload, token) => {
    try {
      ...
      if (confirmResponse.ok && confirmResult.success) {
        ...
        alert(
          `Failed to create user: ${confirmResult.message || "Unknown error"}`
        );
      }
    } catch (error) {
  ...
  const handleUserAction = (userAction, action) => {
    ...
    if (userAction.role === "super_admin" && userAction.id !== user?.id) {
      alert("You cannot modify other Super Admins");
      return;
    }
          ...
          <div className="mx-0 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <StatCard
                ...
                title="Active Now"
                value={
                  analyticsData?.activeUsers || Math.floor(users.length * 0.65)
                }
                trend="+5%"
                color="bg-green-500"
