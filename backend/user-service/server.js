...
app.use((req, res, next) => {
  if (req.method === "OPTIONS") {
    ...
    res.header(
      ...
      "Content-Type, Authorization, X-Confirm, X-Request-ID, x-request-id"
    );
...
// ============ MIDDLEWARE ============
// Apply these ONCE (not twice like in your current code)
...
app.use((req, res, next) => {
  ...
  if (
    req.path.includes("/documents/upload") ||
    req.path.includes("/debug-upload")
  ) {
    return next();
  }
...
const apiLimiter = rateLimit({
  ...
  legacyHeaders: false,
  skip: (req) => {
    // Skip rate limiting for health checks, auth, and static files
    const skipPaths = ['/health', '/ping', '/api-docs', '/Uploads'];
    const isSkipPath = skipPaths.some(path => req.path.startsWith(path));
    const isAuthPath = req.path.includes('/auth/');
    return isSkipPath || isAuthPath;
  }
});
...
app.use(
  ...
  express.static(path.join(__dirname, "Uploads/verification-documents"))
);
...
app.get("/api/debug-paths", async (req, res) => {
  try {
    ...
    const paths = {
      ...
      sharedDbPathFromMiddleware: path.join(
        __dirname,
        "..",
        "..",
        "shared",
    ...
    const checks = {
      ...
      sharedDbExistsFromMiddleware: fileExists(
        paths.sharedDbPathFromMiddleware
      ),
...
// IMPORTANT: Mount verification routes ONLY ONCE and with verifyToken
// This is the key fix for your 404 error
app.use("/api/verification", verifyToken, documentVerificationRoutes);
...
app.listen(PORT, () => {
  ...
  console.log(
    `   - GET  http://localhost:${PORT}/api/test-pending-direct (with auth token)`
  );
  console.log(`=================================`);
});