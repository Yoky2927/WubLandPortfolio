...
const Loader = ({ theme = 'dark', message = 'Loading...', type = 'loading' }) => {
    ...
    // Auto-rotate fun facts only for loading type
    useEffect(() => {
        if (type === 'loading') {
    ...
    return (
        <div className={`fixed inset-0 flex flex-col items-center justify-center z-50 transition-all duration-500 ${
            theme === 'light' ? 'bg-white/80 backdrop-blur-md' : 'bg-black/90 backdrop-blur-md'
        }`}>
            <div className="text-center mb-8 w-full max-w-2xl px-6">
                {/* Icon based on type */}
                {getIcon()}

                {/* Custom Message */}
                <div className={`text-xl font-semibold mb-6 animate-pulse ${getMessageColor()}`}>
                    {message}
                </div>

                {/* Fun Fact Section - Only show for loading type */}
                {type === 'loading' && (
                                    ...
                                    <button
                                        ...
                                        className={`w-3 h-3 rounded-full transition-all duration-300 cursor-pointer ${
                                            index === currentFactIndex
                                                ? theme === 'light' ? 'bg-amber-600 scale-125 shadow-lg' : 'bg-amber-400 scale-125 shadow-lg'
                                                : theme === 'light' ? 'bg-amber-200 hover:bg-amber-400' : 'bg-amber-700 hover:bg-amber-500'
                                        }`}
                ...
                {/* Additional message for success/error types */}
                {type !== 'loading' && (
                    <div className={`text-sm mt-4 ${
                        theme === 'light' ? 'text-gray-600' : 'text-gray-300'
                    }`}>
                        {type === 'success' && 'Redirecting you to the home page...'}
                        {type === 'logout' && 'You have been successfully logged out'}
