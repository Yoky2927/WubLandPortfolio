  ...
  try {
    ...
    const notificationData = {
      ...
      metadata: {
        ...
        entityId: userId,
        verificationType: type.includes("resubmission")
    ...
    const response = await fetch(
      "http://localhost:5001/api/external/notifications",
      {
        method: "POST",
        headers: {
          ...
          "x-internal-token":
            ...
            "communication-service-secret-12345",
        },
    ...
    if (response.ok) {
      ...
      console.warn(
        `⚠️ Failed to send notification to user ${userId}:`,
        errorText
      );
      return false;
    }
...
export const uploadVerificationDocument = async (req, res) => {
  console.log("=== UPLOAD VERIFICATION DOCUMENT START ===");
  ...
  try {
    ...
    const isResubmission =
      req.body.isResubmission === "true" || req.body.isResubmission === true;
    ...
    console.log("User data retrieved:", {
      id: user.id,
      existing_doc_id: user.existing_doc_id,
      existing_doc_status: user.existing_doc_status,
      uploaded_types: user.uploaded_types,
    });

    // Check if document already exists and is not a resubmission
    if (user.existing_doc_id && !isResubmission) {
      if (
        ["pending", "submitted", "reviewing"].includes(user.existing_doc_status)
      ) {
        return res.status(400).json({
          success: false,
    ...
    if (user.existing_doc_id && isResubmission) {
      ...
      console.log("📝 Previous document archived:", previousVersionId);
    }
    ...
    const [result] = await db.query(
      `
            ...
            VALUES (?, ?, ?, ?, 'pending', NOW(), ?)
        `,
      [userId, documentType, documentUrl, filename, previousVersionId]
    );
...
`,
      ...
      console.log(
        `✅ User ${userId} all required documents submitted, verification_status set to 'pending'`
      );
    } else {
      ...
      console.log(
        `📝 User ${userId} still missing documents: ${missingTypes.join(", ")}`
      );
    }
...
export const reviewDocument = async (req, res) => {
  try {
    ...
    if (
      !status ||
      !["approved", "rejected", "needs_resubmission"].includes(status)
    ) {
      return res.status(400).json({
        success: false,
        message:
          "Valid status is required (approved/rejected/needs_resubmission)",
      });
    }
...
export const getPendingVerifications = async (req, res) => {
  try {
    ...
    let countWhereClause =
      "WHERE u.verification_status = 'pending' AND u.role IN ('buyer', 'renter') AND u.has_submitted_documents = true";
    let countParams = [];
...
export const requestResubmission = async (req, res) => {
  try {
    ...
    // Update user verification status - REMOVED resubmission_deadline since it doesn't exist
    await db.query(
      `
            UPDATE users 
            ...
            WHERE id = ?
        `,
      [feedback, adminId, JSON.stringify(history), userId]
    );
    ...
    let notificationMessage = documentType
...
// Add this function to document-verification.controller.js
  ...
  try {
    ...
    await db.query(
      `UPDATE document_verification_records 
       SET status = ?,
           ...
           updated_at = NOW()
       WHERE id = ?`,
      [status, adminId, feedback, rejection_reason || feedback, documentId]
    );
    ...
    switch (status) {
      case 'approved':
        ...
        notificationMessage = `Your ${document.document_type} has been approved.`;
        notificationType = 'success';
        break;
      case 'rejected':
        notificationTitle = 'Document Rejected ✗';
        notificationMessage = `Your ${document.document_type} was rejected. Reason: ${feedback || rejection_reason || 'Please upload a clear document.'}`;
        notificationType = 'error';
        break;
      case 'needs_resubmission':
        notificationTitle = 'Document Needs Resubmission 🔄';
        notificationMessage = `Your ${document.document_type} needs to be resubmitted. Feedback: ${feedback || rejection_reason || 'Please upload a clearer image.'}`;
        notificationType = 'warning';
        break;
      default:
        notificationTitle = 'Document Status Updated';
        notificationMessage = `Your ${document.document_type} status is now ${status}.`;
...
const sendNotificationToAdmins = async (title, message, type = "system") => {
  try {
    // Use the correct endpoint that exists in communication-service
    const notificationData = {
    ...
    const response = await fetch(
      "http://localhost:5001/api/internal/notifications/send-by-role",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      ...
      const errorText = await response.text();
      console.warn(`⚠️ Failed to send admin notification: ${errorText}`);
    }
  } catch (error) {
...
export const getVerificationHistory = async (req, res) => {
  try {
    ...
    const historyWithNames = history.map((entry) => ({
      ...entry,
      admin_first_name: entry.admin_id
        ? adminNames[entry.admin_id]?.first_name || ""
        : "",
      admin_last_name: entry.admin_id
        ? adminNames[entry.admin_id]?.last_name || ""
        : "",
    }));
    ...
    const [result] = await db.query(
      ...
      [
        ...
        specificInstructions.length > 0
          ? JSON.stringify(specificInstructions)
          : null,
        documentId,
      ]
    );
