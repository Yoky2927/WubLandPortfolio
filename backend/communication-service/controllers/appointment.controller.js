...
class AppointmentController {
  ...
  static async createAppointment(req, res) {
    try {
      ...
      if (!appointmentData.title || !appointmentData.scheduled_date || !appointmentData.start_time) {
        return res.status(400).json({
          success: false,
          message: 'Title, scheduled date, and start time are required'
        });
      }
      ...
      // Check time slot availability if broker or property is specified
      if (appointmentData.broker_id || appointmentData.property_id) {
        const isAvailable = await AppointmentModel.isTimeSlotAvailable(
          appointmentData.broker_id || null,
          appointmentData.property_id || null,
          appointmentData.start_time,
          appointmentData.end_time || new Date(new Date(appointmentData.start_time).getTime() + 60 * 60 * 1000) // Default 1 hour
        );
        
        if (!isAvailable) {
          return res.status(409).json({
            success: false,
            message: 'Time slot is not available for the selected broker or property'
          });
        }
      }
      ...
      // Add broker as attendee if specified
      if (appointmentData.broker_id && appointmentData.broker_id !== userId) {
        await AppointmentAttendeeModel.addAttendee(appointmentId, appointmentData.broker_id, {
  ...
  static async getUserAppointments(req, res) {
    try {
      ...
      const appointments = await AppointmentModel.getUserAppointments(userId, filters);
      
      // Get attendee status for each appointment
      const appointmentsWithDetails = await Promise.all(
        appointments.map(async (appointment) => {
          const attendees = await AppointmentAttendeeModel.getAppointmentAttendees(appointment.id);
          return {
            ...appointment,
            attendees,
            current_user_status: attendees.find(a => a.user_id === userId)?.attendee_status || null
          };
        })
      );
  ...
  static async getAppointmentById(req, res) {
    try {
      ...
      const isAttendee = await AppointmentAttendeeModel.isUserAttendee(id, userId);
      ...
      // Get attendees
      const attendees = await AppointmentAttendeeModel.getAppointmentAttendees(id);
    ...
    try {
      ...
      if (updates.start_time || updates.end_time) {
        const isAvailable = await AppointmentModel.isTimeSlotAvailable(
          ...
          updates.end_time || appointment.end_time || new Date(new Date(updates.start_time || appointment.start_time).getTime() + 60 * 60 * 1000),
          id
        );
  ...
  static async updateAppointmentStatus(req, res) {
    try {
      ...
      if (!status || !['scheduled', 'confirmed', 'completed', 'cancelled', 'no_show'].includes(status)) {
        return res.status(400).json({
          success: false,
          message: 'Valid status is required: scheduled, confirmed, completed, cancelled, or no_show'
        });
      }
      ...
      // Update status
      const updated = await AppointmentModel.updateAppointmentStatus(id, status, userId);
  ...
  static async addAttendee(req, res) {
    try {
      const { id } = req.params;
      const { user_id, attendee_role = 'client', send_reminder = true } = req.body;
      ...
      // Check max attendees
      const attendeeCount = await AppointmentAttendeeModel.getAttendeeCount(id);
      if (appointment.max_attendees && attendeeCount.total >= appointment.max_attendees) {
        return res.status(409).json({
          success: false,
      ...
      // Add attendee
      const attendeeId = await AppointmentAttendeeModel.addAttendee(id, user_id, {
        attendee_role,
        attendee_status: 'invited',
  ...
  static async updateAttendeeStatus(req, res) {
    try {
      ...
      if (!status || !['invited', 'confirmed', 'declined', 'attended', 'no_show'].includes(status)) {
        return res.status(400).json({
          success: false,
      ...
      // Update status
      const updated = await AppointmentAttendeeModel.updateAttendeeStatus(appointmentId, userId, status);
  ...
  static async getPropertyAppointments(req, res) {
    try {
      ...
      const appointments = await AppointmentModel.getPropertyAppointments(propertyId, filters);
  ...
  static async getBrokerAppointments(req, res) {
    try {
      ...
      const appointments = await AppointmentModel.getBrokerAppointments(brokerId, filters);
  ...
  static async getUpcomingAppointments(req, res) {
    try {
      ...
      const appointments = await AppointmentModel.getUpcomingAppointments(userId, limit);
  ...
  static async markAttendeeAsAttended(req, res) {
    try {
      ...
      // Mark as attended
      const updated = await AppointmentAttendeeModel.markAsAttended(appointmentId, userId);
      ...
      const attendees = await AppointmentAttendeeModel.getAppointmentAttendees(appointmentId);
      const confirmedAttendees = attendees.filter(a => a.attendee_status === 'confirmed');
      const attendedAttendees = attendees.filter(a => a.attendee_status === 'attended');
      
      if (confirmedAttendees.length > 0 && confirmedAttendees.length === attendedAttendees.length) {
        await AppointmentModel.updateAppointmentStatus(appointmentId, 'completed', organizerId);
      }
