...
import {
  Bell, CheckCircle, AlertCircle, FileText, Shield, X,
  MessageCircle, Sparkles, RefreshCw, Loader as LoaderIcon, Archive,
  ChevronRight, Clock, ShieldCheck, FileCheck, AlertTriangle,
  Upload, FileWarning, ArrowRight, Eye, AlertOctagon
} from 'lucide-react';
...
const NotificationToggle = ({
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        const formattedNotifications = response.data.map(notification => {
          ...
          let specificFeedback = metadata.feedback || metadata.additional_info || '';
          let documentId = metadata.documentId || metadata.entityId || '';

          // Enhance verification notifications with better messages
          switch (notificationType) {
            case 'verification_complete':
              notificationMessage = '🎉 Your identity verification is complete! You can now explore properties.';
              break;
            case 'verification_resubmission':
            case 'document_needs_resubmission':
          ...
          const notificationId = notification.id || notification.notification_uuid || `notif-${Date.now()}-${Math.random()}`;
        ...
        const newUnread = formattedNotifications.filter(n => !n.isRead).length;
        ...
        if (newUnread > 0 && newUnread > oldUnread && !isOpen) {
          ...
          const importantNotifications = formattedNotifications.filter(n =>
            !n.isRead && (
          ...
          importantNotifications.forEach(notif => {
            ...
            } else if (notif.type.includes('resubmission') || notif.type.includes('rejected')) {
  ...
  const markAsRead = async (notificationId) => {
    try {
      ...
      localStorage.setItem('notification_badge_count', newUnreadCount.toString());
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const realNotificationIds = notifications
        .filter(n => !n.isRead && typeof n.id === 'string' && !n.id.startsWith('notif-'))
        .map(n => n.id);
    ...
    switch (notification.type) {
      ...
      case 'document_rejected':
        if (onOpenDocumentUpload) {
          ...
          toast.info('Please upload corrected documents in the verification section', {
            duration: 4000,
            icon: '📄'
          });
        }
  ...
  const getDefaultMessage = (type) => {
    const messages = {
      ...
      'document_needs_resubmission': 'Please resubmit your document with corrections',
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e) => {
      if (e.key === 'notification_badge_count' || e.key === 'user_verification_status') {
        fetchNotifications();
      }
    };
      ...
      {isOpen && (
        <div className="fixed bottom-36 right-20 z-50 w-[450px] max-h-[600px] flex flex-col rounded-2xl shadow-2xl animate-scaleIn overflow-hidden">
          <div className={`flex-1 flex flex-col ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
            {/* Header */}
            <div className={`p-5 border-b ${isDark ? 'border-gray-800 bg-gray-900' : 'border-gray-200 bg-gray-50'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`p-2.5 rounded-xl ${isDark ? 'bg-amber-900/40' : 'bg-amber-100'}`}>
                    <Bell className={`w-6 h-6 ${isDark ? 'text-amber-500' : 'text-amber-600'}`} />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Sparkles className={`w-3.5 h-3.5 ${isDark ? 'text-amber-600' : 'text-amber-500'}`} />
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12">
                  <Archive className={`w-16 h-16 mb-4 ${isDark ? 'text-gray-700' : 'text-gray-300'}`} />
                ...
                <div className="space-y-3">
                  {sortedNotifications.map((notification) => (
                    ...
                    >
                      ...
                      {(notification.type.includes('resubmission') || notification.type.includes('rejected')) && (
                      ...
                      <div className="flex items-start gap-3">
                        <div className={`p-2.5 rounded-lg ${getStatusColor(notification.type)}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center gap-2">
                              ...
                              {(notification.type.includes('resubmission') || notification.type.includes('rejected')) && (
                                <span className={`text-xs ${isDark ? 'text-orange-400/70' : 'text-orange-600/70'}`}>
                                  Click to resubmit →
                                </span>
                              )}
                            </div>

                            {(notification.actionUrl || notification.type === 'message') && (
                              <ChevronRight
                                className={`w-4 h-4 ${
